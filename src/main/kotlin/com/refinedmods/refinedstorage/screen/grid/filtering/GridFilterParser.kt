...
object GridFilterParser {
    fun getFilters(grid: IGrid?, query: String?, filters: List<IFilter<*>>?): List<Predicate<IGridStack>> {
    ...
    private fun getFilters(query: String?): MutableList<Predicate<IGridStack>> {
        ...
        for (part in query!!.toLowerCase().trim { it <= ' ' }.split(" ").toTypedArray()) {
            when {
                part.startsWith("@") -> gridFilters.add(ModGridFilter(part.substring(1)))
                part.startsWith("#") -> gridFilters.add(TooltipGridFilter(part.substring(1)))
                part.startsWith("$") -> gridFilters.add(TagGridFilter(part.substring(1)))
                else -> gridFilters.add(NameGridFilter(part))
            }
        }
        return gridFilters
    }
